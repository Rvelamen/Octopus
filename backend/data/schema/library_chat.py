...
def create_indexes(conn: sqlite3.Connection) -> None:
    conn.execute("CREATE INDEX IF NOT EXISTS idx_lib_chat_sessions_scope ON library_chat_sessions(scope_type, scope_value)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_lib_chat_messages_session ON library_chat_messages(session_id)")
