"""Notes Chat schema for session and message persistence."""
...
def create_tables(conn: sqlite3.Connection) -> None:
    conn.execute("""
        CREATE TABLE IF NOT EXISTS notes_chat_sessions (
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS notes_chat_messages (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            created_at TIMESTAMP DEFAULT (datetime('now', 'localtime')),
            FOREIGN KEY (session_id) REFERENCES notes_chat_sessions(id) ON DELETE CASCADE
        )
    """)
