...
class LibraryHandler(MessageHandler):
    ...
    async def handle(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        if handler:
            ...
            await self._send_error(websocket, message.request_id, f"Unknown library message type: {message.type}")
    ...
    async def _send_success(self, websocket: WebSocket, message: WSMessage, result_type: MessageType, data: dict) -> None:
        await self.send_response(websocket, WSMessage(
            type=result_type,
    ...
    async def _handle_list(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        await self._send_success(websocket, message, MessageType.LIBRARY_LIST_RESULT, {
            "items": items,
    ...
    async def _handle_search(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        await self._send_success(websocket, message, MessageType.LIBRARY_SEARCH_RESULT, {
            "items": items,
            "query": query,
    ...
    async def _handle_add_attachment(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        if not item_id or not temp_path:
            await self._send_error(websocket, message.request_id, "item_id and temp_path are required")
            return
        ...
        try:
            ...
            pass
        await self._send_success(websocket, message, MessageType.LIBRARY_ADD_ATTACHMENT_RESULT, {"attachment": result})
    ...
    async def _handle_annotations_load(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        try:
            ...
            await self._send_success(websocket, message, MessageType.LIBRARY_ANNOTATIONS_LOAD_RESULT, {
                "item_id": item_id,
                "annotations": annotations,
            })
        except Exception as e:
            logger.error(f"Failed to load annotations: {e}")
            await self._send_error(websocket, message.request_id, f"Failed to load annotations: {e}")
    ...
    async def _handle_annotations_save(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        try:
            ...
            await self._send_success(websocket, message, MessageType.LIBRARY_ANNOTATIONS_SAVE_RESULT, {
                "item_id": item_id,
                "saved": result["saved"],
            })
        except Exception as e:
            logger.error(f"Failed to save annotations: {e}")
            await self._send_error(websocket, message.request_id, f"Failed to save annotations: {e}")
    ...
    async def _handle_link_note(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        if not item_id or not note_path:
            await self._send_error(websocket, message.request_id, "item_id and note_path are required")
            return
        result = self.engine.link_note(item_id, note_path, relation)
        await self._send_success(websocket, message, MessageType.LIBRARY_LINK_NOTE_RESULT, {"link": result})
    ...
    async def _handle_collection_list(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        await self._send_success(websocket, message, MessageType.LIBRARY_COLLECTION_LIST_RESULT, {
    ...
    async def _handle_collection_create(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        await self._send_success(websocket, message, MessageType.LIBRARY_COLLECTION_CREATE_RESULT, {"collection": result})
    ...
    async def _handle_collection_update(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        await self._send_success(websocket, message, MessageType.LIBRARY_COLLECTION_UPDATE_RESULT, {"collection": result})
    ...
    async def _handle_collection_delete(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        await self._send_success(websocket, message, MessageType.LIBRARY_COLLECTION_DELETE_RESULT, {"success": True})
    ...
    async def _handle_collection_move(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        await self._send_success(websocket, message, MessageType.LIBRARY_COLLECTION_MOVE_RESULT, {"collection": result})

    async def _handle_collection_add_item(self, websocket: WebSocket, message: WSMessage) -> None:
        item_id = message.data.get("item_id")
        collection_id = message.data.get("collection_id")
        if not item_id or not collection_id:
            await self._send_error(websocket, message.request_id, "item_id and collection_id are required")
            return
        self.engine.add_to_collection(item_id, collection_id)
        await self._send_success(websocket, message, MessageType.LIBRARY_COLLECTION_ADD_ITEM_RESULT, {"success": True})

    async def _handle_collection_remove_item(self, websocket: WebSocket, message: WSMessage) -> None:
        item_id = message.data.get("item_id")
        collection_id = message.data.get("collection_id")
        if not item_id or not collection_id:
            await self._send_error(websocket, message.request_id, "item_id and collection_id are required")
        ...
        await self._send_success(websocket, message, MessageType.LIBRARY_COLLECTION_REMOVE_ITEM_RESULT, {"success": True})
    ...
    async def _handle_import_doi(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        try:
            ...
            await self._send_error(websocket, message.request_id, f"Failed to fetch DOI metadata: {e}")
            return
        ...
        await self._send_success(websocket, message, MessageType.LIBRARY_IMPORT_DOI_RESULT, {"item": item})
    ...
    async def _handle_import_arxiv(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        try:
            ...
            if pdf_path.exists():
                pdf_path.unlink()
            await self._send_error(websocket, message.request_id, f"Failed to import arXiv paper: {e}")
    ...
    async def _handle_search_chunks(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        try:
            ...
            await self._send_success(websocket, message, MessageType.LIBRARY_SEARCH_CHUNKS_RESULT, {
                "results": results,
                "query": query,
            })
        except Exception as e:
    ...
    async def _handle_ai_extract_meta(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        try:
            ...
            await self._send_success(websocket, message, MessageType.LIBRARY_AI_EXTRACT_META_RESULT, {
                "metadata": metadata,
            })
        except Exception as e:
