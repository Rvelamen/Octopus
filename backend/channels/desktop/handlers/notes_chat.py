"""WebSocket message handlers for Desktop channel — Notes Chat handler."""
...
class NotesChatHandler(MessageHandler):
    """Handle Notes chat messages from clients."""
    ...
    def __init__(self, bus, pending_responses: dict[str, asyncio.Queue]):
        ...
        db = Database()
        self.chat_service = NotesChatService(db)
        self.chat_agent = NotesChatAgent(workspace, db)

    async def handle(self, websocket: WebSocket, message: WSMessage) -> None:
        """Process a Notes chat message."""
    ...
    async def _handle_create_session(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        scope_type = message.data.get("scope_type", "global")
        scope_value = message.data.get("scope_value", "")
