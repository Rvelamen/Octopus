"""WebSocket message handlers for Desktop channel — Library Chat handler."""
...
from backend.agent.library_chat_agent import LibraryChatAgent
from backend.data.database import Database
from backend.utils.helpers import get_workspace_path


class LibraryChatHandler(MessageHandler):
    """Handle Library chat messages from clients."""
    ...
    def __init__(self, bus, pending_responses: dict[str, asyncio.Queue]):
        ...
        db = Database()
        self.chat_service = LibraryChatService(db)
        self.chat_agent = LibraryChatAgent(workspace, db)

    async def handle(self, websocket: WebSocket, message: WSMessage) -> None:
        """Process a Library chat message."""
    ...
    async def _handle_chat(self, websocket: WebSocket, message: WSMessage) -> None:
        ...
        try:
            ...
            logger.error(f"[LibraryChatHandler] Chat failed: {e}")
            await self.send_response(websocket, WSMessage(
                type=MessageType.ERROR,
                request_id=request_id,
