...
class MessageBus:
    ...
    def __init__(self):
        ...
        self._event_subscribers: list[Callable[[AgentEvent], Awaitable[None]]] = []
        self._running = False
    ...
    async def publish_event(self, event: AgentEvent) -> None:
        """Publish an agent event (e.g. start, finish, chunk)."""
        await self.events.put(event)
    
    async def consume_event(self) -> AgentEvent:
    ...
    async def dispatch_events(self) -> None:
        """Dispatch events to subscribers (e.g. Desktop Channel)."""
        self._running = True
        while self._running:
            try:
