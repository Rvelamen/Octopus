 ...
 *
 * Performance:
 * - `markdownComponents` is memoized once per hook instance so that
 *   `renderMessageContent` does not allocate fresh component objects on each
 *   call. This matters when MessageList re-renders during streaming.
 * - `setWorkspacePreviewPath` is the React state setter (stable reference),
 *   so the components closure does not need to track it as a dep.
 */
export function useMessageRenderer() {
  ...
  const markdownComponents = useMemo(
    ...
    [] // setWorkspacePreviewPath is React's stable state setter
  );
