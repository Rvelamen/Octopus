 ...
 * 
 * Based on Obsidian's graph simulation engine.
 * Uses D3-force physics with optional WASM acceleration.
 * Runs in Web Worker to avoid blocking main thread.
 */

// D3-force simulation code (minified from graph.html)
const SIM_CODE = `( () => {
    "use strict";
    function A(A) {
        return function() {
    ...
    function() {
        ...
        for (var A = 0; A < M.length; ++A) {
            var t = M[A];
            null == t.fx ? t.x += t.vx *= .18 : (t.x = t.fx,
            t.vx = 0),
            null == t.fy ? t.y += t.vy *= .18 : (t.y = t.fy,
            t.vy = 0)
        }
    }
    ), O = null, X = 1e3 / 60, L = function(A) {
    ...
    if (m) {
        ...
        Y = function() {
            ...
            Z && Z.byteLength === A || (Z = new SharedArrayBuffer(A));
            var t = L(Z)
              , n = new Uint32Array(Z,Z.byteLength - 4,1)
              , i = n[0];
            n[0]++,
            postMessage({
                id: t,
...
// Create internal worker from the simulation code
const workerBlob = new Blob([SIM_CODE], { type: 'application/javascript' });
const internalWorker = new Worker(URL.createObjectURL(workerBlob));

// Forward messages between main thread and internal worker
self.onmessage = (e) => {
    internalWorker.postMessage(e.data);
};

internalWorker.onmessage = (e) => {
    self.postMessage(e.data);
};

internalWorker.onerror = (e) => {
